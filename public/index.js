...
class keyboard {
    ...
    getHTML() {
        ...
        for (const [key, item] of Object.entries(this.data)) {
            ...
            switch (item.text) {
                case 'space':
                    html += `<div class="btn-box ${hasSymbol ? 'space-2' : 'space'}" data-no="${key}"><div class="item" value="${item.text}">${item.text}</div>
                    </div>`
                    break;
                case 'up': case 'back': case 'face':
                    html += `<div class="btn-box icon" data-no="${key}"><div class="item" value="${item.text}">${item.icon}</div></div>`
                    break;
                case '123':
                ...
                case 'symbol':
                    hasSymbol = true
                    html += `<div class="btn-box icon" data-no="${key}"><div class="connect-word">${item.content.map(c => `<div class="word" value="${c}">${c}</div>`).join('')}</div>
                        <div class="item" value="${item.content[0]}">${item.content[0]}</div>
                        <div class="top-word">${item.content[0]}</div>
                        </div>`
                    break;
                case 'global': case 'mic':
                    html += `<div class="btn-box bottom" data-no="${key}"><div class="item" value="${item.text}">${item.icon}</div></div>`
                    break;
                default:
                    html += `<div class="btn-box" data-no="${key}">
            ...
            switch (text) {
                case "up":
                    Object.values(this.nodes.btns).map(btn => {
                        ...
                        if(this.data[btn.parentNode.getAttribute('data-no')].content_up) {
                            if (!this.isup) {
                                btn.parentNode.querySelector('.connect-word').innerHTML = this.data[btn.parentNode.getAttribute('data-no')].content_up.map(c => `<div class="word" value="${c}">${c}</div>`).join('')
                            }else {
                                btn.parentNode.querySelector('.connect-word').innerHTML = this.data[btn.parentNode.getAttribute('data-no')].content.map(c => `<div class="word" value="${c}">${c}</div>`).join('')
                            }
                        }
                    })
                    text = ''
